...
#
# [REQUIRED] Add support for the MAVLink tx1 communications protocol.
# tx1 mavlink message analyzed by patrick

MAVLINKPATH_TX1_REL = libs/mavlink/include/mavlink
MAVLINKPATH_TX1 = $$BASEDIR/$$MAVLINKPATH_TX1_REL

exists($$MAVLINKPATH_TX1/tx1/tx1){
    message(Including support for Tx1 library)
}else {
    error($$sprintf("MAVLink Tx1 folder does not exist at '%1'! Run 'git submodule init && git submodule update' on the command line.",$$MAVLINKPATH_TX1))
}
