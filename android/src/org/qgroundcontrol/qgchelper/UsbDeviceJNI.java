...
public class UsbDeviceJNI extends QtActivity implements TextToSpeech.OnInitListener
{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(USBBroadcastReceiver, new IntentFilter("android.hardware.usb.action.USB_STATE"));
        ...
        public void onReceive(Context context, Intent intent)
        {
           if(intent.getAction().equals("android.hardware.usb.action.USB_STATE"))
           {
                 if ((intent.getExtras().getBoolean("connected")))
                 {
                     if(flag == 0)
                     {
                    Log.d("==tgl_debug","usb be insert ");
                    flag=1;
                    AlertDialog.Builder builder = new AlertDialog.Builder(UsbDeviceJNI.this);
                    builder.setMessage("检测到usb，打开usb共享功能");
                    builder.setTitle("提示");
