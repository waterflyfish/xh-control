...
# -------------------------------------------------
DebugBuild {
    message("Skipping Installer Framework, But A Result Of Debug Building")
}

ReleaseBuild {
    WindowsBuild {
        message("Installer Framework")
        DEFINES+=QGC_ENABLE_ADMIN
        contains(DEFINES, QGC_ENABLE_ADMIN) {
               message("Installer QGC_ENABLE_ADMIN")
               QMAKE_LFLAGS += /MANIFESTUAC:\"level=\'requireAdministrator\' uiAccess=\'false\'\"
        }

        INSTALL_DESTDIR  = $${OUT_PWD}
        DESTDIR_COPY_INSTALL_LIST = $$replace(INSTALL_DESTDIR,"/","\\")
        BASEDIR_COPY_INSTALL_LIST = $$replace(BASEDIR,"/","\\")

        INSTALL_COPY_FILE_LIST = \
            $$BASEDIR_WIN\\license.txt \
            $$BASEDIR_WIN\\installheader.bmp \

       message($$DESTDIR_COPY_INSTALL_LIST)
       for(COPY_FILE, INSTALL_COPY_FILE_LIST) {
          QMAKE_POST_LINK += $$escape_expand(\\n) $$QMAKE_COPY \"$$COPY_FILE\" \"$$DESTDIR\"
       }

       QMAKE_POST_LINK += $$escape_expand(\\n) $$QMAKE_COPY_DIR \"$$BASEDIR_COPY_INSTALL_LIST\\deploy\" \"$$DESTDIR_COPY_INSTALL_LIST\\deploy\"
    }
}
