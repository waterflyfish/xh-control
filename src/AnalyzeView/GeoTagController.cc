...
{
    QString filename = QGCFileDialog::getOpenFileName(NULL, tr("Select log file load"), QString(), "PX4 log file (*.px4log);;All Files (*.*)");
    if (!filename.isEmpty()) {
...
{
    QString dir = QGCFileDialog::getExistingDirectory(NULL, tr("Select image directory"));
    if (!dir.isEmpty()) {
