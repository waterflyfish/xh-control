...
void FollowMe::followMeHandleManager(const QString&)
{
    ...
    for (int i=0; i< vehicles.count(); i++) {
        ...
        bool _connectlost = vehicle->connectionLost();
        if (vehicle->px4Firmware() && vehicle->flightMode().compare(PX4FirmwarePlugin::followMeFlightMode, Qt::CaseInsensitive) == 0 && !_connectlost) {
            _enable();
            return;
        }
    }
...
void FollowMe::_enable()
{
    ...
    _gcsMotionReportTimer.setInterval(_toolbox->qgcPositionManager()->updateInterval() == 0 ? 1000 : _toolbox->qgcPositionManager()->updateInterval());
...
void FollowMe::setGPSLocation(QGeoPositionInfo geoPositionInfo)
{
    ...
    qDebug()<<"update:" << _motionReport.lat_int << _motionReport.lon_int ;
...
void FollowMe::_sendGCSMotionReport(void)
{
    ...
    for (int i=0; i< vehicles.count(); i++) {
        ...
        if(vehicle->flightMode().compare(PX4FirmwarePlugin::followMeFlightMode, Qt::CaseInsensitive) == 0 && !_connectlost) {
            qDebug()<<"follow target:"<<i<<"    lat:"<<follow_target.lat<<"lon:"<<follow_target.lon<<"alt:"<<follow_target.alt<<"vx:"<<follow_target.vel[0]<<"vy:"<<follow_target.vel[1];
