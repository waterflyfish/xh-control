...
bool QGCApplication::languageflag=false;
    ...
    , _lastKnownHomePosition(37.803784, -122.462276, 0.0)
{
    ...
    _app = this;
#ifndef __mobile__
    QGCObject *obj = new QGCObject;
    _app->installEventFilter (obj);
#endif
...
void QGCApplication::_initCommon(void)
{
    ...
    qmlRegisterType<QGCUpdate>      ("QGroundControl.Palette", 1, 0, "QGCUpdate");
    ...
    qmlRegisterType<Tx1Controller>                      ("QGroundControl.Controllers", 1, 0, "Tx1Controller");
...
bool QGCApplication::_initForNormalAppBoot(void)
{
    ...
    connect(mainWindow,&MainWindow::seed,this,&QGCApplication::geet);
    connect(mainWindow,&MainWindow::seeed,this,&QGCApplication::geeet);
    ...
    do{
        ...
        if (!saveFilename.isEmpty()) {
            ...
            if(QFile::exists(saveFilename) && !QFile::remove(saveFilename)){
                ...
                saveError = true;
                QGCMessageBox::warning(tr("File Error"),tr("Could not overwrite existing file.\nPlease provide a different file name to save to."));
            } else{
                bool sucess = false;
                sucess = QFile::copy(tempLogfile, saveFilename);
                if(!sucess){
                    //if(!QFile::copy(tempLogfile, saveFilename)) {
                    ...
                    QFileInfo fileInfo(tempLogfile);
                    if(tempDir == fileInfo.dir ()){
                        //QFile::remove(tempLogfile);
                    }
                    break;
                }
...
void QGCApplication::geeet(){
    QSettings setting;
    languageflag=false;
    setting.setValue("language",languageflag);
}
void QGCApplication::geet(){
    QSettings setting;
    languageflag=true;
    setting.setValue("language",languageflag);
}
