...
#include "CustomCommandWidgetAdd.h"
...
//const char* CompanyName = QT_TRANSLATE_NOOP("MainWindow","xianghongkeji");
...
static const char *rgDockWidgetNames[] = {
    ...
    QT_TRANSLATE_NOOP("MainWindow","Analyze"),
	  ...
	  //=======================================
//edit by wang.lichen
enum DockWidgetTypesAdd {
    Chinese,
    English
};
static const char *rgDockWidgetNamesAdd[] = {
    QT_TR_NOOP("Chinese"),
    QT_TR_NOOP("English")
};
//=========================================
...
//    , _appChineseName ("智慧蜂地面站 V3.0.1")
    , _appEnglishName("QGC--BrainyBee V3.0.1")
    ...
    ,_autopilot(NULL)
{
    Q_ASSERT(_instance == NULL);
    _instance = this;
    _appChineseName = QStringLiteral ("智慧蜂地面站V3.0.1");
    //-- Load fonts
    ...
    connect(_ui.actionLoadParameters,&QAction::triggered,this,&MainWindow::loadFromFilePicker);
...
void MainWindow::_buildCommonWidgets(void)
{
    ...
    for (int i = 0, end = ARRAY_SIZE(rgDockWidgetNamesAdd); i < end; i++) {

        const char* pDockWidgetNameAdd = rgDockWidgetNamesAdd[i];

        // Add to menu
        QAction* action = new QAction(tr(pDockWidgetNameAdd), this);
        action->setCheckable(false);
        action->setData(i);
        connect(action, &QAction::triggered, this, &MainWindow::_showDockWidgetActionAdd);
        _ui.menuLanguage->addAction(action);
        _mapName2Action[pDockWidgetNameAdd] = action;
    }
}
...
//========================================================
void MainWindow::_showDockWidgetActionAdd(bool show)
{
    Q_UNUSED(show);
    QAction* action = qobject_cast<QAction*>(QObject::sender());
    if(!action){
...
#else
    QMessageBox msgBox;
    msgBox.setWindowTitle (tr("GroundControl"));
    msgBox.setText(tr("If you want to change the language, you should have to restart the QGC!"));
    ...
    case QMessageBox::Ok:
        /*
         * just shutdown,does not judge there is a activeVehicle
         */
//        restartApp();
        // Save was clicked
        break;
...
#endif
    //_showDockWidgetAdd(rgDockWidgetNamesAdd[action->data().toInt()], show);
}
#endif
...
void MainWindow::restartApp()
{
    ...
    int time = 5;
    while(time){
        process.startDetached (qgcApp ()->applicationFilePath ());
        bool restart = process.waitForStarted ();
        if(!restart){
            time--;
            qDebug()<<"QGC can not restart,try again";
        }else{
            break;
        }
    }
...
void MainWindow::loadSettings()
{
    ...
    QSettings settings;
    bool language = false;
    language = settings.value ("language",language).toBool ();
//    qDebug()<<"load settings: "<<language;
    settings.beginGroup(MAIN_SETTINGS_GROUP);
    _lowPowerMode   = settings.value("LOW_POWER_MODE",      _lowPowerMode).toBool();
    _showStatusBar  = settings.value("SHOW_STATUSBAR",      _showStatusBar).toBool();

    if(language){
        _appEnglishName = settings.value ("APPENGLISHNAME", _appEnglishName).toString();
        _applicationname = _appEnglishName;
//        qDebug()<<"load settings english"<<_appEnglishName;
    }else{
        QByteArray appname;
        QTextCodec *codec = QTextCodec::codecForName ("UTF-8");
        appname  = settings.value ("APPCHINESENAME",appname).toByteArray();
        if(!codec->toUnicode (appname).isEmpty ()){
            _appChineseName = codec->toUnicode (appname);
        }
        _applicationname = _appChineseName;
//        qDebug()<<"load settings chinses:"<<_appChineseName;
    }
    settings.endGroup();
}
...
//    qDebug()<<"language:"<<language;
    ...
    QTextCodec *codec = QTextCodec::codecForName("UTF-8");
    QByteArray appchinesename = codec->fromUnicode (_appChineseName);
    if(language){
        settings.setValue ("APPENGLISHNAME", _appEnglishName);
//        qDebug()<<"english store:"<<_appEnglishName;
    }else{
        settings.setValue ("APPCHINESENAME", appchinesename);
//        qDebug()<<"chinese store:"<<appchinesename;
    }
    settings.endGroup();
...
{
        QString windowname = _applicationname;//qApp->applicationName() + tr("--BrainyBEE Ground Station");
        setWindowTitle (_applicationname);
}

void MainWindow::loadFromFilePicker()
{
    ...
    if(_vehicle){
        ...
        qgcApp()->showMessage(QString("NO Vechicle connect to QGC,Please check your Vehicle is working well!"));
        return;
    }
#ifndef __mobile__
    QString errors;
...
void MainWindow::_showDockWidgetAdd(const QString& name, bool show)
{
    // Create the inner widget if we need to
    QAction *action = _mapName2Action[name];
        ...
        if (!_mapName2DockWidget.contains(name)) {
            if(!_createInnerDockWidgetAdd(name)) {
                qWarning() << "Trying to load non existing widget:" << name;
                return;
            }
        }
...
bool MainWindow::_createInnerDockWidgetAdd(const QString& widgetName)
{
    QGCDockWidget* widget = NULL;
    QAction *action = _mapName2Action[widgetName];
    if(action) {
        ...
        case Chinese:
             ...
             widget = new CustomCommandWidgetAdd(widgetName, action, this);
            break;
        case English:
             widget = new CustomCommandWidgetAdd(widgetName, action, this);
            break;
