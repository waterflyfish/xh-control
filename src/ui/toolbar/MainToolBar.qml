...
Rectangle {
    ...
    property bool progressBarVisibled : false
    property bool firmwarenew : _controller.firmwareNew
        ...
        Rectangle {
            ...
            Column {
                ...
                GridLayout {
                    ...
                    QGCLabel{text:(activeVehicle && activeVehicle.battery.current.value != -1)?(activeVehicle.battery.current.valueString + " " + activeVehicle.battery.current.units) : "N/A" }
                    ...
                    QGCLabel { text: qsTr("Generating Current:")}
                    QGCLabel { text:(activeVehicle && activeVehicle.battery.voltages.value != -1 ) ? (activeVehicle.battery.voltages.valueString + " " + activeVehicle.battery.current.units) : "N/A"}
                }
            }
    ...
    MessageDialog{
        id: firmwareNew
        title : qsTr("New Firmware")
        text : qsTr("There is a latest version , you need to update")
        modality: Qt.ApplicationModal
        icon : StandardIcon.Information
        standardButtons: StandardButton.Ok | StandardButton.No
        detailedText:  _controller.detailsText
        visible :_controller.firmwareNew
        onRejected: {
            firmwareNew.close()
        }
    }
