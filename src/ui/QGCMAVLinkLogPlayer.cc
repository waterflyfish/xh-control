...
#endif
void QGCMAVLinkLogPlayer::_playbackPercentCompleteChanged(double percentComplete)
{
    _ui->positionSlider->blockSignals(true);
    _ui->positionSlider->setValue((int)percentComplete);
    if(percentComplete<= 100 && percentComplete >= 0){
        double seconds = _logDurationSeconds * (1.0 * percentComplete / 100.0);
        int secondsPart  = seconds;
        int minutesPart  = seconds / 60;
        int hoursPart    = seconds / 60 / 60;
        secondsPart -= 60 * minutesPart;
        minutesPart -= 60 * hoursPart;

        QString currentTime = QString("%1h:%2m:%3s").arg(hoursPart, 2).arg(minutesPart, 2).arg(secondsPart, 2);
        _ui->logStatsLeftLabel->setText(currentTime);
    }
    _ui->positionSlider->blockSignals(false);
}
