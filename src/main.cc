...
#ifdef __android__
    if (!QMetaType::isRegistered(QMetaType::type("QGCSerialPortInfo")))
        qRegisterMetaType<QGCSerialPortInfo>();
#endif
...
#pragma warning( disable : 4930 4101 )
    ...
    QSettings setting;
    bool language;
    if(!setting.value("language",language).toBool()){
        translator.load(":/qgc_zh_CN.qm");
        app->installTranslator(&translator);
    }
#ifndef __mobile__
    QPixmap pixmap(":/splash/resources/screen.jpg");
...
#endif
    {
        if (!app->_initForNormalAppBoot()) {
            qDebug()<<"_initForNormalAppBoot failed!";
            return -1;
        }
#ifndef __mobile__
...
//        QCoreApplication::processEvents();
        screen.finish(MainWindow::instance());
#endif
        exitCode = app->exec();
    }
