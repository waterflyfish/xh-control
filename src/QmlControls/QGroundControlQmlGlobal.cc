...
QMap<QString, FactMetaData*>& QGroundControlQmlGlobal::nameToMetaDataMap(void) {
    ...
    return map;
	 ...
	 //====================================================
void QGroundControlQmlGlobal::setLanguageIndex(int id)
{
    _languageIndex = id;
    QSettings setting;

    if(_languageIndex==1) setting.setValue("language",false);
    if(_languageIndex==2) setting.setValue("language",true);
