...
bool FirmwareImage::load(const QString& imageFilename, uint32_t boardId)
{
    ...
    if (imageFilename.endsWith(".bin") || imageFilename.endsWith(".txt")) {
        _binFormat = true;
        ...
        emit statusMessage(tr("Unsupported file format"));
        return false;
    }
}
...
bool FirmwareImage::_ihxLoad(const QString& ihxFilename)
{
    ...
    if (!ihxFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        emit statusMessage(QString(tr("Unable to open firmware file %1, error: %2")).arg(ihxFilename).arg(ihxFile.errorString()));
        return false;
    }
    ...
    while (true) {
        ...
        if (!(recordType == 0 || recordType == 1)) {
            emit statusMessage(QString(tr("Unsupported record type in file: %1")).arg(recordType));
            return false;
        }
...
bool FirmwareImage::_px4Load(const QString& imageFilename)
{
    ...
    if (!px4File.open(QIODevice::ReadOnly | QIODevice::Text)) {
        emit statusMessage(QString(tr("Unable to open firmware file %1, error: %2")).arg(imageFilename).arg(px4File.errorString()));
        return false;
    }
    ...
    if (!JsonHelper::validateRequiredKeys(px4Json, requiredKeys, errorString)) {
        emit statusMessage(QString(tr("Firmware file mission required key: %1")).arg(errorString));
        return false;
    }
    ...
    if (!JsonHelper::validateKeyTypes(px4Json, keys, types, errorString)) {
        emit statusMessage(QString(tr("Firmware file has invalid key: %1")).arg(errorString));
        return false;
    }

    uint32_t firmwareBoardId = (uint32_t)px4Json.value(_jsonBoardIdKey).toInt();
    if (firmwareBoardId != _boardId) {
        emit statusMessage(QString(tr("Downloaded firmware board id does not match hardware board id: %1 != %2")).arg(firmwareBoardId).arg(_boardId));
        return false;
    }
    ...
    if (success) {
        ...
        if (parameterFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            ...
            if (bytesWritten != decompressedBytes.count()) {
                emit statusMessage(QString(tr("Write failed for parameter meta data file, error: %1")).arg(parameterFile.errorString()));
                parameterFile.close();
            ...
            emit statusMessage(QString(tr("Unable to open parameter meta data file %1 for writing, error: %2")).arg(parameterFilename).arg(parameterFile.errorString()));
        }
    ...
    if (success) {
        ...
        if (airframeFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            ...
            if (bytesWritten != decompressedBytes.count()) {
                ...
                emit statusMessage(QString(tr("Write failed for airframe meta data file, error: %1")).arg(airframeFile.errorString()));
                airframeFile.close();
            ...
            emit statusMessage(QString(tr("Unable to open airframe meta data file %1 for writing, error: %2")).arg(airframeFilename).arg(airframeFile.errorString()));
        }
    }
    ...
    if (!decompressFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        emit statusMessage(QString(tr("Unable to open decompressed file %1 for writing, error: %2")).arg(decompressFilename).arg(decompressFile.errorString()));
        return false;
    }
    
    qint64 bytesWritten = decompressFile.write(decompressedBytes);
    if (bytesWritten != decompressedBytes.count()) {
        emit statusMessage(QString(tr("Write failed for decompressed image file, error: %1")).arg(decompressFile.errorString()));
        return false;
    }
                                         ...
                                         QByteArray&		decompressedBytes)	///< Returned decompressed bytes
{
    ...
    if (!jsonObject.contains(sizeKey)) {
        emit statusMessage(QString(tr("Firmware file missing %1 key")).arg(sizeKey));
        return false;
    }
    int decompressedSize = jsonObject.value(QString(sizeKey)).toInt();
    if (decompressedSize == 0) {
        emit statusMessage(QString(tr("Firmware file has invalid decompressed size for %1")).arg(sizeKey));
        return false;
    }
    ...
    if (parts.count() == 1) {
        emit statusMessage(QString(tr("Could not find compressed bytes for %1 in Firmware file")).arg(bytesKey));
        return false;
    }
    parts = parts.last().split("\"");
    if (parts.count() == 1) {
        emit statusMessage(QString(tr("Incorrectly formed compressed bytes section for %1 in Firmware file")).arg(bytesKey));
        return false;
    }
    ...
    if (decompressedBytes.count() == 0) {
        emit statusMessage(QString(tr("Firmware file has 0 length %1")).arg(bytesKey));
        return false;
    }
    if (decompressedBytes.count() != decompressedSize) {
        emit statusMessage(QString(tr("Size for decompressed %1 does not match stored size: Expected(%1) Actual(%2)")).arg(decompressedSize).arg(decompressedBytes.count()));
        return false;
    }
    
    emit statusMessage(QString(tr("Successfully decompressed %1")).arg(bytesKey));
...
bool FirmwareImage::_binLoad(const QString& imageFilename)
{
    ...
    if (!binFile.open(QIODevice::ReadOnly)) {
        emit statusMessage(QString(tr("Unabled to open firmware file %1, %2")).arg(imageFilename).arg(binFile.errorString()));
        return false;
    }
