...
void PX4FirmwareUpgradeThreadWorker::_startFindBoardLoop(int board)
{
    _foundBoard = false;
    _findBoardFirstAttempt = true;
    _board = board;
    ...
    if (_findBoardFromPorts(portInfo, boardType)) {
        if (!_foundBoard) {
            ...
            if(boardType == QGCSerialPortInfo::BoardTypeSerial
                    ...
                    || _board != 0){
                emit _controller->startUpgradeFirmware(portInfo, boardType);
                return;
            }
...
void PX4FirmwareUpgradeThreadController::startFindBoardLoop(int board)
{
    qCDebug(FirmwareUpgradeLog) << "PX4FirmwareUpgradeThreadController::findBoard";
    emit _startFindBoardLoopOnThread(board);
}
