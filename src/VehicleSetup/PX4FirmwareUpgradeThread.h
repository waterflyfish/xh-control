...
private slots:
    void _init(void);
    void _startFindBoardLoop(int board);
    void _reboot(void);
    void _flash(void);
...
private:
    ...
    bool                _board;                 ///< 0:px4 or apm, other: gps and so on
};
...
public:
    ...
    void startFindBoardLoop(int board);
...
signals:
    ...
    void flashComplete(void);
     ...
     /// @brief Signalled to update progress for upload firmware file commands
    void startUpgradeFirmware(const QGCSerialPortInfo &portInfo, int type);
    ...
    void _startFindBoardLoopOnThread(int board);
