...
ConsoleUpgradeController::ConsoleUpgradeController(QObject* parent)
  :QObject(parent)
  ...
  ,_rebootGpsTimes(0)
{
  connect(&_ackTimer, &QTimer::timeout, this, &ConsoleUpgradeController::_ackTimeout);
  connect(&_rebootGpsTimer, &QTimer::timeout, this, &ConsoleUpgradeController::_sendBootReadyCommand);
...
void ConsoleUpgradeController::_openAckResponse(Request* openAck)
{
...
void ConsoleUpgradeController::_closeDownloadSession(bool success)
{
...
void ConsoleUpgradeController::_closeUploadSession(bool success)
{
    if (!success){
        _rebootStatus = false;
...
void ConsoleUpgradeController::_downloadAckResponse(Request* readAck, bool readFile)
{
...
void ConsoleUpgradeController::_listAckResponse(Request* listAck)
{
...
void ConsoleUpgradeController::_createAckResponse(Request* createAck)
{
...
void ConsoleUpgradeController::_writeAckResponse(Request* writeAck)
{
...
void ConsoleUpgradeController::_writeFileDatablock(void)
{
...
void ConsoleUpgradeController::_receiveBytes(LinkInterface* link, mavlink_message_t message){
    if (message.sysid != 255 && message.sysid != 0) {
        return;
    }
...
void ConsoleUpgradeController::_receiveMessage(mavlink_message_t message)
{
...
void ConsoleUpgradeController::listDirectory(const QString& dirPath)
{
...
void ConsoleUpgradeController::_fillRequestWithString(Request* request, const QString& str)
{
...
void ConsoleUpgradeController::_sendListCommand(void)
{
    Request request;
...
void ConsoleUpgradeController::downloadPath(const QString& from, const QDir& downloadDir)
{
    if (_currentOperation != kCOIdle) {
        _emitErrorMessage(tr("Command not sent. Waiting for previous command to complete."));
        return;
...
void ConsoleUpgradeController::streamPath(const QString& from, const QDir& downloadDir)
{
    if (_currentOperation != kCOIdle) {
        _emitErrorMessage(tr("Command not sent. Waiting for previous command to complete."));
        return;
...
void ConsoleUpgradeController::_downloadWorker(const QString& from, const QDir& downloadDir, bool readFile)
{
    if (from.isEmpty()) {
        return;
    }
...
void ConsoleUpgradeController::uploadPath(LinkInterface *link, const QString& toPath, const QFileInfo& uploadFile, int selectedFirmware){
    Q_UNUSED(link);
...
///     @param toPath File in UAS to upload to, fully qualified path
///     @param uploadFile Local file to upload from
void ConsoleUpgradeController::uploadPath(const QString& toPath, const QFileInfo& uploadFile)
{
...
QString ConsoleUpgradeController::errorString(uint8_t errorCode)
{
    switch(errorCode) {
    case kErrNone:
...
bool ConsoleUpgradeController::_sendOpcodeOnlyCmd(uint8_t opcode, OperationState newOpState)
{
    if (_currentOperation != kCOIdle) {
...
bool ConsoleUpgradeController::_setupAckTimeout(void)
{
...
void ConsoleUpgradeController::_clearAckTimeout(void)
{
    _ackTimes = 0;
    _ackTimer.stop();
}
...
void ConsoleUpgradeController::stopAckTimeout()
{
    if(_ackTimer.isActive())
    {
        _ackTimer.stop ();
...
void ConsoleUpgradeController::_ackTimeout(void)
{
    ...
    if(_resetStatus) {
        qDebug() << "ConsoleUpgradeController _sendResetCommand timeout, now send it again";
        _sendResetCommand();
        return;
    }
    switch (_currentOperation) {
...
void ConsoleUpgradeController::_sendResetCommand(void)
{
    qDebug() << "ConsoleUpgradeController::_sendResetCommand excuted";
    Request request;
...
void ConsoleUpgradeController::_emitErrorMessage(const QString& msg)
{
    /// print error message
...
void ConsoleUpgradeController::_emitListEntry(const QString& entry)
{
    emit listEntry(entry);
}

/// @brief Used when update firmware by wireless connection
void ConsoleUpgradeController::_sendSearchVersionCommand(LinkInterface * link, int selectedComponent){
    Q_UNUSED(link);
    _sendSearchVersionCommand(selectedComponent);
}

/// @brief Used when update firmware by wireless connection
void ConsoleUpgradeController::_sendSearchVersionCommand(int selectedComponent) {
    qDebug() << "ConsoleUpgradeController _sendSearchVersionCommand_ send : " << _selectedComponent;
    if(_selectedComponent == -1)
    {
...
void ConsoleUpgradeController::_sendBootReadyCommand() {
...
void ConsoleUpgradeController::_sendRequest(Request* request)
{
   if(!_setupAckTimeout()){
       return ;
   }
...
void ConsoleUpgradeController::_sendMessageOnLink(LinkInterface* link, mavlink_message_t message)
{
    // Make sure this is still a good link
    //MAVLinkProtocol* _mavlink = qgcApp()->toolbox()->mavlinkProtocol();
    //static const uint8_t messageKeys[256][7] = MAVLINK_MESSAGE_CRCS;
    if(link){
