...
void ParameterLoader::_valueUpdated(const QVariant& value)
{
    ...
    if (fact->rebootRequired() && !qgcApp()->runningUnitTests()) {
        qgcApp()->showMessage(QString(tr("Change of parameter %1 requires a Vehicle reboot to take effect")).arg(name));
    }
}
        ...
        if (!line.startsWith("#")) {
            ...
            if (wpParams.size() == 5) {
                if (_vehicle->id() != lineMavId) {
                    return QString(tr("The parameters in the stream have been saved from System Id %1, but the current vehicle has the System Id %2.")).arg(lineMavId).arg(_vehicle->id());
                }
...
void ParameterLoader::_checkInitialLoadComplete(bool failIfNoDefaultComponent)
{
    ...
    if (initialLoadFailures) {
        qgcApp()->showMessage(tr("QGroundControl was unable to retrieve the full set of parameters from the vehicle. "
        ...
        if (!qgcApp()->runningUnitTests()) {
            qCWarning(ParameterLoaderLog) << tr("The following parameter indices could not be loaded after the maximum number of retries: ") << indexList;
        }
    ...
    if (_defaultComponentId == FactSystem::defaultComponentId && !_defaultComponentIdParam.isEmpty()) {
        qgcApp()->showMessage(tr("QGroundControl did not receive parameters from the default component. "
                                 "This will cause QGroundControl to be unable to display its full user interface. "
                                 "If you are using modified firmware, you may need to resolve any vehicle startup errors to resolve the issue. "
                                 "If you are using standard firmware, you may need to upgrade to a newer version to resolve the issue."));
        if (!qgcApp()->runningUnitTests()) {
            qCWarning(ParameterLoaderLog) << tr("Default component was never found, param:") << _defaultComponentIdParam;
        }
        emit parametersReady(true /* missingParameters */);
        return;
    }
...
void ParameterLoader::_initialRequestTimeout(void)
{
    if (!_vehicle->genericFirmware()) {
        ...
        qgcApp()->showMessage(tr("Vehicle did not respond to request for parameters, retrying"));
    }
...
QString ParameterLoader::parameterMetaDataFile(MAV_AUTOPILOT firmwareType, int wantedMajorVersion, int& majorVersion, int& minorVersion)
{
    ...
    if (cacheFile.exists()) {
        ...
        if (wantedMajorVersion != cacheMajorVersion) {
            qWarning() << tr("Parameter meta data cache corruption:") << cacheFile.fileName() << "major version does not match file name" << "actual:excepted" << cacheMajorVersion << wantedMajorVersion;
        } else {
    ...
    if (!cacheHit) {
        ...
        if (cacheHitIndex != -1) {
            ...
            if (majorVersion != cacheMajorVersion) {
                qWarning() << tr("Parameter meta data cache corruption:") << cacheFile.fileName() << tr("major version does not match file name") << tr("actual:excepted") << majorVersion << cacheMajorVersion;
                cacheHit = false;
            } else {
    ...
    if(!m) {
        internalMetaDataFile = plugin->internalParameterMetaDataFileAdd();
    }

    if(m) {
        internalMetaDataFile = plugin->internalParameterMetaDataFile();
    }
