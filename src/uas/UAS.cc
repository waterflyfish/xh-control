...
//            qDebug()<<latitude<<longitude<<"+++++";
        ...
        case MAVLINK_MSG_ID_STATUSTEXT:
        {
            ...
            if(!m){
                if(text == "TRANSITION_DENIED: ARMING_STATE_INIT - ARMING_STA")
                    text = "拒绝解锁状态初始化";
                if(text == "REJECTING component arm cmd")
                    text = "拒绝组件解锁命令";
                if(text == "Rejecting arming cmd")
                    text = "拒绝执行解锁命令";
                if(text == "Not ready to fly: Sensors not set up correctly")
                    text = "不能飞行 传感器未设置正确";

                if(text == "PREFLIGHT FAIL: NO MAG SENSOR #0")
                    text = "起飞失败 传感器未设置正确";

                if(text == "Warning: Primary compass not found")
                    text = "警告 罗盘未被发现";

                if(text == "PREFLIGHT FAIL: GYRO #0 UNCALIBRATED")
                    text = "起飞失败 未标定陀螺仪";

                if(text == "Warning: Primary gyro not found")
                    text = "警告 未发现陀螺仪";

                if(text == "PREFLIGHT FAIL: MAG #0 UNCALIBRATED")
                    text = "起飞失败 未校准磁场计";

                if(text == "PREFLIGHT FAIL: AIRSPEED SENSOR MISSING")
                    text = "起飞失败 未发现空速传感器";

                if(text == "Operation timeout")
                    text = "操作超时";
                if(text == "PREFLIGHT FAIL: ACCEL #0 UNCALIBRATED")
                    text = "起飞失败 加速度计未校准";
                if(text == "ARMING DENIED: Flying with USB is not safe")
                    text = "解锁失败 飞行时使用有线串口不安全";

                if(text.leftRef(30)== "NOT ARMING: Avionics power low")
                    text = "不能解锁 飞控功率过低";

                if(text.leftRef(27)== "CAUTION: Avionics power low")
                    text = "注意 飞控功率过低";

                if(text == "ARMING DENIED: VERY LOW BATTERY")
                    text = "不能解锁 电池电量过低";

                if(text == "Takeoff denied, disarm and re-try")
                    text = "拒绝起飞 上锁重试";

                if(text == "AIRSPEED WARNING: WIND OR CALIBRATION ISSUE")
                    text = "空速计警告";

                if(text == "data link #0 regained")
                    text = "数据链路 #0 恢复";
            }
...
/* virtaul and key control the attitude
 */
//void UAS::setVirtualControlSetpoint(float roll, float pitch, float yaw, float thrust, float channel10)
//{
...
//    static float manualChannel10 = 0.0;
...
//             (!qIsNaN (channel10) && channel10 != manualChannel10)) {
//        sendCommand = true;
...
//        manualYawAngle = yaw;
//        manualThrust = thrust;
//        manualChannel10 = channel10;

//        mavlink_message_t message;
//        const float axesScaling = 1.0 * 500.0;
//        //Calculate the new commands for roll pitch yaw and channel10
//        const float newRollCommand = roll * axesScaling + 1500;
//        const float newPitchCommand = -pitch * axesScaling + 1500;
//        const float newYawCommand = yaw * axesScaling  + 1500;
//        const float newThrustCommand = -thrust * axesScaling + 1500;

//        mavlink_msg_rc_channels_pack_chan (mavlink->getSystemId()
//                                           ,mavlink->getComponentId()
//                                           ,0
//                                           ,&message
//                                           ,1
...
//                                           ,65535
//                                           ,100);
//        qDebug()<<"roll"<<newRollCommand<<"pitch"<<newPitchCommand<<"yaw"<<newYawCommand<<"thrust"<<newThrustCommand<<" channel10"<<channel10;
    ...
    if (sendCommand) {
        ...
        if (joystickMode == Vehicle::JoystickModeAttitude) {
            ...
            mavlink_msg_set_position_target_local_ned_pack_chan(mavlink->getSystemId(),
                                                                ...
                                                                yaw,
                                                                0);
        qDebug()<<"roll"<<py<<"pitch"<<px<<"yaw"<<yaw<<"thrust"<<pz<<"buttons: "<<buttons;
...
//                                               ,65535
//                                               ,100);
            qDebug()<<"roll"<<newRollCommand<<"pitch"<<newPitchCommand<<"yaw"<<newYawCommand<<"thrust"<<newThrustCommand<<"buttons: "<<buttons;
        }
