...
SimpleMissionItem::SimpleMissionItem(Vehicle* vehicle, QObject* parent)
    ...
    , _altitudeRelativeToHomeFact   (0, tr("Altitude is relative to home"), FactMetaData::valueTypeUint32)
    , _supportedCommandFact         (0, tr("Command:"),                     FactMetaData::valueTypeUint32)
...
QString SimpleMissionItem::abbreviation() const
{
    ...
    case MavlinkQmlSingleton::MAV_CMD_DO_CHANGE_SPEED:
        return QStringLiteral("S");
    case MavlinkQmlSingleton::MAV_CMD_DO_PARACHUTE:
        return QStringLiteral("PR");
    case MavlinkQmlSingleton::MAV_CMD_DO_SET_CAM_TRIGG_DIST:
        return QStringLiteral("TR");
    case MavlinkQmlSingleton::MAV_CMD_DO_VTOL_TRANSITION:
        if(_missionItem.param1() == 4)
...
QmlObjectListModel* SimpleMissionItem::textFieldFacts(void)
{
    ...
    if (rawEdit()) {
        ...
        if (specifiesCoordinate() && !altitudeAdded && !_homePositionSpecialCase) {
            _missionItem._param5Fact.setRawValue(tr("Latitude:"));
            _missionItem._param5Fact.setMetaData(_latitudeMetaData);
            _latitudeMetaData->setRawUnits("");
            model->append(&_missionItem._param5Fact);
            _missionItem._param6Fact.setRawValue(tr("Longitude:"));
            _missionItem._param6Fact.setMetaData(_longitudeMetaData);
            _longitudeMetaData->setRawUnits("");
            model->append(&_missionItem._param6Fact);
...
void SimpleMissionItem::setParam1(double param)
{
    if (param != _missionItem.param1()) {
        _missionItem.setParam1 (param);
    }
}

void SimpleMissionItem::setParam2(double param)
{
    if (param != _missionItem.param2()) {
        _missionItem.setParam2 (param);
    }
}

void SimpleMissionItem::setParam3(double param)
{
    if (param != _missionItem.param3()) {
        _missionItem.setParam3 (param);
    }
}

void SimpleMissionItem::setParam4(double param)
{
    if (param != _missionItem.param4()) {
        _missionItem.setParam4 (param);
    }
}
...
//=============================================================
void SimpleMissionItem::setFlage(bool flage)
{
    _missionItem.setFlage(flage);
}
