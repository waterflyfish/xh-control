...
QVariantList MissionCommandTree::getCommandsForCategory(Vehicle* vehicle, const QString& category)
{
    ...
    foreach (MAV_CMD command, commandMap.keys()) {
        ...
        if (uiInfo->category() == category) {
            ...
            _jsondat.append(QVariant::fromValue(uiInfo));
...
void MissionCommandTree::setToolboxAdd(QGCToolbox* toolbox)
{
    QGCTool::setToolbox(toolbox);
    //_buildAvailableCommandsAdd(vehicle);
#ifdef UNITTEST_BUILD
    if (_unitTest) {
        // Load unit testing tree
        _staticCommandTree[MAV_AUTOPILOT_GENERIC][MAV_TYPE_GENERIC] =           new MissionCommandList(":/json/unittest/MavCmdInfoCommon.json", true, this);
        ...
        foreach (MAV_AUTOPILOT firmwareType, _toolbox->firmwarePluginManager()->knownFirmwareTypes()) {
            ...
            foreach(MAV_TYPE vehicleType, vehicleTypes) {
                QString overrideFile = plugin->missionCommandOverridesAdd(vehicleType);

                if (!overrideFile.isEmpty()) {
                    _staticCommandTree[firmwareType][vehicleType]->_loadMavCmdInfoJson(overrideFile,firmwareType == MAV_AUTOPILOT_GENERIC && vehicleType == MAV_TYPE_GENERIC);
...
#endif
//vehicle->_loadMetaData(":/json/Vehicle/VehicleFact.json");
}
void MissionCommandTree::_buildAvailableCommandsAdd(Vehicle* vehicle)
{
    MAV_AUTOPILOT   baseFirmwareType;
    MAV_TYPE        baseVehicleType;
...
void MissionCommandTree::setToolboxAddTwo(QGCToolbox* toolbox)
{
