...
#endif
    // BingMaps
    _versionGoogleSatelliteChina = "160000";
...
UrlFactory::_getURL(MapType type, int x, int y, int zoom, QNetworkAccessManager* networkManager)
{
    ...
    case GoogleMapChina:
    {
        ...
        QString sec2    = ""; // after &zoom=...
        _getSecGoogleWords(x, y, sec1, sec2);
        return QString("http://%1%2.google.cn/%3/lyrs=m@%4&hl=%5&gl=cn&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(_getServerNum(x,y,4)).arg(request).arg(_versionGoogleSatelliteChina).arg(_language).arg(x).arg(sec1).arg(y).arg(zoom).arg(sec2);
    }
    break;
    case GoogleSatelliteChina:
    {
        ...
        _getSecGoogleWords(x, y, sec1, sec2);
//        _tryCorrectGoogleVersions(networkManager);
        return QString("http://%1%2.google.cn/%3/lyrs=s@%4&gl=cn&x=%5%6&y=%7&z=%8&s=%9").arg(server).arg(_getServerNum(x, y, 4)).arg(request).arg(_versionGoogleSatelliteChina).arg(x).arg(sec1).arg(y).arg(zoom).arg(sec2);
    }
    break;
    case GoogleTerrainChina:
    {
        ...
        QString sec2    = "";
        _getSecGoogleWords(x, y, sec1, sec2);
        return QString("http://%1%2.google.cn/%3/lyrs=t@1343,r@8343&hl=%4&gl=cn&x=%5%6&y=%7&z=%8&s=%9").arg(server).arg(_getServerNum(x,y,4)).arg(request).arg(_language).arg(x).arg(sec1).arg(y).arg(zoom).arg(sec2);
    }
    break;
#endif
    case StatkartTopo:
