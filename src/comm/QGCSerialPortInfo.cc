...
QGCSerialPortInfo::BoardType_t QGCSerialPortInfo::boardType(void) const
{
    ...
    if (boardType == BoardTypeUnknown) {
        ...
        } else if (manufacturer() == "FTDI" || description() == "USB Serial Port" /* start*/ || description().endsWith("UART Bridge")/* end*/) {
            qCDebug(QGCSerialPortInfoLog) << "Found possible USBSerial (by name matching fallback)";
            boardType = BoardTypeSerial;
#ifdef __android__
