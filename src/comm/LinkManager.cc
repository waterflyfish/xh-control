...
void LinkManager::_addLink(LinkInterface* link)
{
    ...
    connect(link, &LinkInterface::mavMessageReceived,   _mavlinkProtocol,   &MAVLinkProtocol::receiveMavMessasge);
...
void LinkManager::_updateAutoConnectLinks(void)
{
    ...
    if (!foundUDP && _autoconnectUDP) {
        ...
        qDebug() << "New auto-connect UDP port added";
        ...
        if (boardType != QGCSerialPortInfo::BoardTypeUnknown) {
            ...
            if (_autoconnectConfigurationsContainsPort(portInfo.systemLocation())) {
                ...
                case QGCSerialPortInfo::BoardTypeSerial:
                    if (_autoconnect3DRRadio) {
                        pSerialConfig = new SerialConfiguration(QString ("USB Serial on %1").arg(portInfo.portName().trimmed()));
                    }
                    break;
				//end
#ifndef __mobile__
                ...
                if (pSerialConfig) {
                    ...
                    pSerialConfig->setBaud((boardType == QGCSerialPortInfo::BoardTypeSikRadio || boardType == QGCSerialPortInfo::BoardTypeSerial) ? 115200 : 115200);
...
void LinkManager::_activeLinkCheck(void)
{
    ...
    if (!found && link) {
        ...
        qgcApp()->showMessage(foundNSHPrompt ?
                                  QString(tr("Please check to make sure you have an SD Card inserted in your Vehicle and try again.")) :
                                  QString(tr("Your Vehicle is not responding. If this continues shutdown QGroundControl, restart the Vehicle letting it boot completely, then start QGroundControl.")));
    }
}
#endif
